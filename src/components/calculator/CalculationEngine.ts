...
export const calculateDrillingParams = (
  od: number, 
  wt: number, 
  grade: string, 
  depth: number, 
  ...
  maxTorqueInput: number, 
  bitTorqueInput: number,
  weight: string
): DrillingCalc => {
  const yieldStrength = API_PIPE_GRADES[grade as keyof typeof API_PIPE_GRADES].yield * 6.895;
...
export const calculateRunningParams = (
  od: number, 
  depth: number, 
  mudWt: number, 
  pipeWt: number, 
